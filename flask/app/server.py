...
def splash():
    ...
    if playTone:
        ...
        play_tone(frequency=frequency, duration = duration,
...
def play_tone(frequency=60, duration=30, gain=1, loops=1):
    """
    ...
    """
    sendmsg(frequency, duration, gain, loops)
...
def sendmsg(frequency, duration, gain, loops):
    print("sending message")
    try:
        connection = socket.bind(playerURL)
        time.sleep(0.5)
        cmd_dict = {'action':'play', 'frequency':frequency, 'duration':duration,
                    'gain':gain, 'loops':loops}
        cmd_dict = json.dumps(cmd_dict)
        loaded_json = json.loads(cmd_dict)
        socket.send_json(loaded_json)
